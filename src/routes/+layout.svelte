...
<div class="min-h-screen text-slate-100 flex font-body bg-ink relative">
	...
	<main class="flex-1 w-full flex flex-col h-screen overflow-hidden">
		{#if !$focusMode}
			<!-- Top Bar -->
			<header class="h-10 border-b border-white/10 flex items-center justify-end px-6 bg-black/10">
				<div class="flex items-center gap-2">
					...
					<button type="button" onclick={() => (contactOpen = true)} class="text-xs font-bold uppercase tracking-[0.18em] text-white/85 border border-white/20 bg-white/5 px-3 py-1.5 rounded hover:bg-white/15 hover:text-white transition-all cursor-pointer">{t('nav.contact', $language)}</button>
				</div>
			</header>
		{/if}
