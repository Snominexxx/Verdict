...
<div class="h-full flex flex-col text-white">
	...
	<header class="border-b border-white/10 bg-black/20 px-6 py-4 flex items-center justify-between">
		<div>
			<h2 class="text-sm font-bold uppercase tracking-wider text-white">{t('court.hub', $language)}</h2>
			<p class="text-xs text-white/50 mt-0.5">{t('court.description', $language)}</p>
		</div>
		<div class="flex gap-3 items-center">
			<div class="flex gap-4 text-[11px] font-mono uppercase text-white/60">
			...
			<a href="/cases" class="px-3 py-1.5 bg-white text-black text-[11px] font-bold uppercase tracking-widest rounded hover:bg-white/90 transition">
				{t('court.stageNew', $language)}
			</a>
			<a href="/library" class="px-3 py-1.5 border border-white/20 text-[11px] font-bold uppercase tracking-widest rounded text-white/70 hover:bg-white/10 transition">
	...
	<section class="flex-1 overflow-y-auto px-6 py-6 space-y-8">
		...
		<div class="space-y-3">
			<div class="flex items-center justify-between">
				<div class="flex items-baseline gap-3">
					<h3 class="text-xs font-bold uppercase tracking-widest text-white/60">{t('court.ongoingCases', $language)}</h3>
				</div>
				{#if ongoingCases.length}
					<p class="text-[11px] text-white/40 font-mono">{t('court.lastUpdated', $language)} {formatRelativeTime(ongoingCases[0]?.updatedAt)}</p>
				{/if}
			</div>

			{#if ongoingCases.length === 0}
				<div class="border border-dashed border-white/15 rounded-lg p-5 text-center text-white/40">
					<p class="text-xs">{t('court.emptyOngoing', $language)}</p>
				</div>
			{:else}
				<div class="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
					{#each ongoingCases as entry (entry.id)}
						<article class="border border-white/10 bg-white/[0.03] rounded-xl p-4 flex flex-col gap-3">
							<div class="flex items-start justify-between gap-2">
								<div class="min-w-0">
									<p class="text-[10px] uppercase tracking-[0.2em] text-white/40">{roleCopy[entry.role]?.[$language] ?? t('court.litigant', $language)}</p>
							...
							<p class="text-xs text-white/60 leading-relaxed line-clamp-2">{summarize(entry.synopsis, 140)}</p>
							...
							<div class="flex gap-2">
								<button type="button" on:click={() => resumeCase(entry)} class="flex-1 text-center bg-white text-black py-1.5 rounded text-[11px] font-bold uppercase tracking-widest hover:bg-white/90 transition">
									{t('court.reenter', $language)}
								</button>
								<button type="button" on:click={() => finalizeCase(entry.id)} class="flex-1 text-center border border-white/15 py-1.5 rounded text-[11px] font-bold uppercase tracking-widest text-white/60 hover:text-white hover:border-flare/50 transition">
		...
		<div class="space-y-3">
			<div class="flex items-center justify-between">
				<div class="flex items-baseline gap-3">
					<h3 class="text-xs font-bold uppercase tracking-widest text-white/60">{t('court.finishedCases', $language)}</h3>
				</div>
				{#if finishedCases.length}
					<p class="text-[11px] text-white/40 font-mono">{t('court.lastClosure', $language)} {formatRelativeTime(finishedCases[0]?.updatedAt)}</p>
				{/if}
			</div>

			{#if finishedCases.length === 0}
				<div class="border border-dashed border-white/15 rounded-lg p-5 text-center text-white/40">
					<p class="text-xs">{t('court.emptyFinished', $language)}</p>
				</div>
			{:else}
				<div class="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
					{#each finishedCases as entry (entry.id)}
						<article class="border border-white/5 bg-black/20 rounded-xl p-4 flex flex-col gap-3">
							<div class="flex items-start justify-between gap-2">
								<div class="min-w-0">
									<p class="text-[10px] uppercase tracking-[0.2em] text-white/40">{roleCopy[entry.role]?.[$language] ?? t('court.litigant', $language)}</p>
									<h4 class="text-sm font-semibold leading-snug mt-0.5 truncate">{entry.title}</h4>
							...
							{#if entry.performance}
								<div class="border border-white/10 rounded-lg p-3 bg-white/[0.02] space-y-2">
									<div class="flex items-center justify-between">
										<p class="text-[10px] uppercase tracking-[0.2em] text-white/40">{t('court.performance', $language)}</p>
										<p class="text-base font-mono font-bold text-white">{entry.performance.scores.average}%</p>
									</div>
									<p class="text-[11px] text-white/60 leading-relaxed line-clamp-2">{entry.performance.summary}</p>
									<div class="grid grid-cols-3 sm:grid-cols-5 gap-1.5 text-[10px] font-mono">
										<div class="p-1.5 border border-white/10 rounded bg-black/25 text-center">
											<p class="text-white/40 truncate">{t('debate.metricPersuasion', $language)}</p>
											<p class="text-white/80 font-bold">{entry.performance.scores.persuasion}%</p>
										</div>
										<div class="p-1.5 border border-white/10 rounded bg-black/25 text-center">
											<p class="text-white/40 truncate">{t('debate.metricLawCited', $language)}</p>
											<p class="text-white/80 font-bold">{entry.performance.scores.lawCited}%</p>
										</div>
										<div class="p-1.5 border border-white/10 rounded bg-black/25 text-center">
											<p class="text-white/40 truncate">{t('debate.metricStructure', $language)}</p>
											<p class="text-white/80 font-bold">{entry.performance.scores.structure}%</p>
										</div>
										<div class="p-1.5 border border-white/10 rounded bg-black/25 text-center">
											<p class="text-white/40 truncate">{t('debate.metricResponsiveness', $language)}</p>
											<p class="text-white/80 font-bold">{entry.performance.scores.responsiveness}%</p>
										</div>
										<div class="p-1.5 border border-white/10 rounded bg-black/25 text-center">
											<p class="text-white/40 truncate">{t('debate.metricFactFidelity', $language)}</p>
											<p class="text-white/80 font-bold">{entry.performance.scores.factFidelity}%</p>
										</div>
									</div>
								</div>
							{:else}
								<div class="border border-dashed border-white/15 rounded-lg p-3 bg-black/15">
									<p class="text-[11px] text-white/50">{t('court.noScoreRecorded', $language)}</p>
								</div>
							{/if}
							<div class="flex gap-2 text-[10px] text-white/50 font-mono">
